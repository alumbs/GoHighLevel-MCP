...
import { attachMcpHttp } from './mcp-http';
...
export class GHLMCPHttpServer {
  ...
  constructor() {
    ...
    // Attach new MCP HTTP endpoint
    attachMcpHttp(this.app, this);
  }
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/tools', async (req, res) => {
      try {
        ...
        res.json({
          tools: [...contactTools, ...conversationTools, ...blogTools, ...opportunityTools, ...calendarTools, ...emailTools, ...locationTools, ...emailISVTools, ...socialMediaTools, ...mediaTools, ...objectTools, ...associationTools, ...customFieldV2Tools, ...workflowTools, ...surveyTools, ...storeTools, ...productsTools, ...paymentsTools, ...invoicesTools],
  ...
  private isPaymentsTool(toolName: string): boolean {
    const paymentsToolNames = [
      'ghl_create_payment', 'ghl_list_payments', 'ghl_get_payment', 'ghl_update_payment',
      'ghl_delete_payment', 'ghl_bulk_update_payments', 'ghl_create_payment_method', 'ghl_list_payment_methods',
      'ghl_get_payment_method', 'ghl_update_payment_method', 'ghl_delete_payment_method', 'ghl_list_payment_gateways',
      'ghl_get_payment_gateway', 'ghl_update_payment_gateway', 'ghl_delete_payment_gateway'
    ];
  ...
  private isInvoicesTool(toolName: string): boolean {
    const invoicesToolNames = [
      'ghl_create_invoice', 'ghl_list_invoices', 'ghl_get_invoice', 'ghl_update_invoice',
      'ghl_delete_invoice', 'ghl_bulk_update_invoices', 'ghl_create_invoice_item', 'ghl_list_invoice_items',
      'ghl_get_invoice_item', 'ghl_update_invoice_item', 'ghl_delete_invoice_item'
    ];
    return invoicesToolNames.includes(toolName);
  }

  /**
   * Get all available tools for MCP HTTP endpoint
   */
  async getAllTools() {
    try {
      const contactToolDefinitions = this.contactTools.getToolDefinitions();
      ...
      console.error('[GHL MCP HTTP] Error getting tools:', error);
      throw error;
    }
  }

  /**
   * Execute a tool by name for MCP HTTP endpoint
   */
